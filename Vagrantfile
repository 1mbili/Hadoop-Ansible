...
ENV["LC_ALL"] = "en_US.UTF-8"
 #Prepared keys for ansible
public_key_path = ENV['ansible_public_key']
#TODO FIX Vagrant keys
...
Vagrant.configure("2") do |config|
  (1..config_data['datanode_count']).each do |i|
    config.vm.define "dn#{i}" do |datanode|
      ...
      datanode.vm.provision "file", source: public_key_path, destination: "~/.ssh/ansible.pub"
